...
W.view.common.ImageView = Backbone.View.extend({
    ...
    templates: {
        imageView: '#image-view-tmpl-src',
        multiImageView: '#multi-image-view-tmpl-src',
        oneImageView: '#one-image-view-tmpl-src',
    },
    ...
    initialize: function(options) {
        ...
        this.isShowHeightAlert = options.isShowHeightAlert || true;
    ...
    computeWidthAndHeight: function(options){
        ...
        if(this.autoShowHelp){
            if (this.help) {
                ...
                var items = ["<span class='help-block mt15' >上传图片建议尺寸 ", this.width];
                if (this.isShowHeightAlert == true) {
                    items.push("*");
                    items.push(this.height);
                }else{
                    items.push(' 宽度');
                }
                if (this.sizeLimit) {
        ...
        this.imgTmpl = _.template('<img class="img-thumbnail" src="<%= url %>" width="<%= width %>px" height="<%= height %>px" style="width: <%= width %>px; height: <%= height %>px" />');
    ...
    render: function() {
        ...
        var $node = this.renderTmpl('imageView', {
            picUrl: this.picUrl,
            width: this.width,
            height: this.height
        });
    ...
    onAdd: function() {
        var $node = this.renderTmpl('oneImageView', {
            picUrl: this.picUrl,
            width: this.width,
            height: this.height
        });
        this.$el.find('.multi-image-zome').prepend($node);
    },
...
W.registerUIRole('input[data-ui-role="image-selector"]', function() {
    ...
    // 是否显示高度提示
    var isShowHeightAlert = $imageInput.attr("data-is-show-height-alert");
    ...
    var view = new W.view.common.ImageView({
        ...
        isShowHeightAlert: isShowHeightAlert
    });
